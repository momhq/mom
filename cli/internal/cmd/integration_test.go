...
func assertMCPEntry(t *testing.T, mcpPath string) {
	...
	if mom["command"] != "mom" {
		t.Errorf("%s mcpServers.mom.command = %v, want mom", mcpPath, mom["command"])
	}
}
...
func TestIntegration_InitAllRuntimes(t *testing.T) {
	...
	allRuntimes := []string{"claude", "codex", "cline", "openclaude", "windsurf"}
	...
	if len(enabled) != 5 {
		t.Errorf("expected 5 enabled runtimes, got %d: %v", len(enabled), enabled)
	}
	...
	assertFileContains(t, settingsJSON, "mom record")
	...
	// ── Cline ──
	clineMD := filepath.Join(dir, ".clinerules", "mom-context.md")
	assertFileExists(t, clineMD)
	assertFileContains(t, clineMD, "Generated by MOM")

	clineHook := filepath.Join(dir, ".clinerules", "hooks", "task-complete.sh")
	assertFileExists(t, clineHook)
	info, err := os.Stat(clineHook)
	if err == nil && info.Mode()&0111 == 0 {
		t.Errorf("cline hook %s is not executable", clineHook)
	}
	...
	assertFileContains(t, windsurfHooks, "mom record")
...
func TestIntegration_UpgradeRegistersAllRuntimes(t *testing.T) {
	...
	assertFileContains(t, filepath.Join(dir, ".claude", "settings.json"), "mom record")
	...
	assertFileExists(t, filepath.Join(dir, ".clinerules", "mom-context.md"))
	assertFileExists(t, filepath.Join(dir, ".clinerules", "hooks", "task-complete.sh"))
...
func TestIntegration_MCPConfigCorrectness(t *testing.T) {
	...
	if mom["command"] != "mom" {
		t.Errorf("mcpServers.mom.command = %v, want mom", mom["command"])
	}
