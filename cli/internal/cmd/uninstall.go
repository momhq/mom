...
func runUninstall(cmd *cobra.Command, args []string) error {
	...
	// Confirm with user.
	if !yes {
		cmd.Println("This will remove all MOM files from this project:")
		if hasLeoDir {
			cmd.Println("  - .mom/ (config, memory, cache)")
		}
		for _, adapter := range adapters {
			for _, f := range adapter.GeneratedFiles() {
				cmd.Printf("  - %s (if generated by MOM)\n", f)
			}
		}
		cmd.Println()
	...
	cmd.Println("\n  MOM has been removed from this project.")
	return nil
}
