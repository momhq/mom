...
func coreConstraints() map[string]string {
  ...
  "updated_by": "system",
  "content": {
    "constraint": "Every session leaves one session-log doc in .leo/logs/. MOM produces logs at wrap-up, never consumes them — monitoring is external.",
  ...
  "content": {
    ...
    "responsibility": "MOM is solely responsible for propagation. Specialists report to MOM, only MOM writes to the KB.",
    "anti_patterns": [
