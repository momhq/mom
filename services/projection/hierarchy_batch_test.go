...
func (s *icmStub) Fold(_ context.Context, in FoldInput) (FoldResult, error) {
	...
	case has(in.Existing, "_l0_hint"):
		...
		files["episodes/"+cid+".md"] = PrependFrontmatter(
			Frontmatter{Type: typeEpisode, Level: 0, Version: 1, Sources: offs}, "# Episode\n")
...
// TestHierarchicalL1Batches verifies that L1 synthesis is batched (not one
// giant call over every episode) and that the ICM layout — reference/ +
// identity.md, no chronological layer — is produced.
func TestHierarchicalL1Batches(t *testing.T) {
	...
	// chunkSize 1 → one episode per event → 25 episodes.
	...
	// 25 episodes / batch 20 → 2 L1 batches (not a single all-episodes call).
	if stub.l1 != 2 {
		t.Errorf("want 2 L1 batches, got %d", stub.l1)
	}
	if stub.l2 != 1 {
