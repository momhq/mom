...
func buildPrompt(in FoldInput) (string, bool) {
	...
	b.WriteString("2. Produce RESIDUE ONLY: decisions, preferences, corrections, recurring procedures. Drop chatter and transient status.\n")
	b.WriteString("3. Incrementally UPDATE existing files rather than regenerating from scratch.\n")
	b.WriteString("4. Do NOT restate CLAUDE.md content. Set \"claude_block\" to empty string always.\n")
	b.WriteString("5. Every file MUST begin with YAML frontmatter: level (0=episode,1=topic/timeline), kind, sources (ledger offsets), tags, time_range_start, time_range_end (RFC3339).\n")
	b.WriteString("6. SCOPE: only create topic files for subjects DIRECTLY worked on in THIS project. If another project or codebase is merely mentioned in passing, do NOT create a topic file for it — ignore it.\n\n")
	b.WriteString("OUTPUT FORMAT — return ONLY this JSON, no prose, no code fence:\n")
	b.WriteString(`{"files":[{"path":"topics/voice.md","content":"---\nlevel: 1\nkind: topic\nsources: [1,2,3]\ntags: [voice]\ntime_range_start: 2026-01-01T00:00:00Z\ntime_range_end: 2026-01-31T23:59:59Z\n---\n# Topic: Voice\n..."}],"index":"# MOM Vault Index\n...","claude_block":""}` + "\n\n")
