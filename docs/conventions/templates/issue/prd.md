...
1. **PM Manager** writes the PRD as a PR adding `docs/prds/YYYY-MM-DD-slug/prd.md`. Owner reviews on the PR. When merged, this issue moves `PRD` → `RDD` on the board.
2. **Engineer Manager** (or relevant technical Manager) writes the RDD as a PR adding `docs/rdds/YYYY-MM-DD-slug/rdd.md`. Owner reviews. When merged, issue moves to `Backlog`.
...
<!-- Owner's raw thoughts, constraints, references, links. The PM pulls from here when writing the PRD doc. -->
