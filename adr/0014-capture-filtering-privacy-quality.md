# 0014 — Drafter filtering: hard redaction for secrets, silent drop for noise
...
Drafter applies two filter layers at the write boundary, before a turn becomes a memory. The filters live inside Drafter — no other component runs content-shaped judgement on incoming turns.
...
**User override is per-path, not per-turn.** Explicit user-authored writes (`mom remember "…"` on the CLI, explicit `mom_record` calls over MCP) bypass both layers entirely — the user's explicitness wins over Drafter's heuristics. The filter pipeline only runs on automatic transcript capture.
...
- The vault contains substance, not chatter, and contains substance with credentials redacted in place. Finder quality improves because there is less noise to outscore and less risk of surfacing a leaked key.
- `filter_audit` is owned by Drafter and is the single place to read "how often did the hard filter fire and on what categories" without ever storing what it caught. The matched secret never lands on disk.
...
- Drafter filtering is the only place where MOM exercises content-shaped judgement on incoming turns. Finder, curation, and storage stay deterministic and content-agnostic; Finder does not re-run filters at read time.
...
- **No Drafter filtering; rely on the user to redact.** Rejected: secrets routinely appear in conversation without the user noticing in time. A default-on filter is the only reliable place to catch them.
