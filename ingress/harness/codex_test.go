...
func TestCodexAdapter_RegisterHooks(t *testing.T) {
	...
	// RegisterHooks also enables Codex's hook system via config.toml.
	toml, err := os.ReadFile(filepath.Join(dir, ".codex", "config.toml"))
	if err != nil {
		t.Fatalf("reading config.toml: %v", err)
	}
	if !strings.Contains(string(toml), "hooks = true") {
		t.Error("config.toml missing [features] hooks = true")
	}
	if strings.Contains(string(toml), "[mcp_servers.mom]") {
		t.Error("config.toml must not contain an MCP server entry")
	}
}

// RegisterHooks normalizes a config.toml that has duplicate [features]
// blocks and the deprecated codex_hooks flag, leaving exactly one
// [features] hooks = true and preserving unrelated sections.
func TestCodexAdapter_RegisterHooks_NormalizesDuplicateFeaturesBlocks(t *testing.T) {
	dir := t.TempDir()
...
// RegisterHooks preserves unrelated config and adds [features] without
// touching trust levels.
func TestCodexAdapter_RegisterHooks_PreservesExisting(t *testing.T) {
	dir := t.TempDir()
	...
	if !strings.Contains(content, "trust_level") {
		t.Error("RegisterHooks should preserve existing config content")
	}
	if !strings.Contains(content, "hooks = true") {
		t.Error("RegisterHooks should enable the hooks feature")
	}
}
...
func TestCodexAdapter_GeneratedFiles(t *testing.T) {
	...
	expected := []string{"AGENTS.md", filepath.Join(".codex", "hooks.json"), filepath.Join(".codex", "config.toml")}
...
func TestCodexAdapter_NoIdentity(t *testing.T) {
	...
	content, _ := os.ReadFile(filepath.Join(dir, "AGENTS.md"))
	if !strings.Contains(string(content), ".mom/vault/INDEX.md") {
...
// RegisterHooks strips a stale pre-v0.50 [mcp_servers.mom] block (MOM no
// longer ships an MCP server) while preserving unrelated config and keeping
// the [features] hooks flag.
func TestCodexAdapter_RegisterHooks_StripsLegacyMCPBlock(t *testing.T) {
	dir := t.TempDir()
	codexDir := filepath.Join(dir, ".codex")
	if err := os.MkdirAll(codexDir, 0o755); err != nil {
		t.Fatal(err)
...
`
	...
	if strings.Contains(content, "[mcp_servers.mom]") {
		t.Fatalf("legacy MCP block should be stripped:\n%s", content)
	}
	if strings.Contains(content, "MOM_PROJECT_DIR") || strings.Contains(content, "/tmp/mom-dev") {
		t.Fatalf("legacy MCP env should be dropped:\n%s", content)
	}
	if !strings.Contains(content, "trust_level") {
		t.Fatalf("unrelated config should be preserved:\n%s", content)
	}
	if !strings.Contains(content, "hooks = true") {
		t.Fatalf("hooks feature should be enabled:\n%s", content)
	}
}
